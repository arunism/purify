...
class SimplifiedBERTHallucinationReducer:
    def __init__(
        self, model_name="bert-base-uncased", embedding_model="all-MiniLM-L6-v2"
    ):
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.model = AutoModelForMaskedLM.from_pretrained(model_name).to(self.device)
        self.tokenizer = AutoTokenizer.from_pretrained(model_name)
...
def compare_responses(query, basic_model, advanced_model):
    ...
    print(f"Basic BERT Response (generated in {basic_time:.2f} seconds):")
...
if __name__ == "__main__":
    basic_model = BasicBERT()
    advanced_model = SimplifiedBERTHallucinationReducer()
